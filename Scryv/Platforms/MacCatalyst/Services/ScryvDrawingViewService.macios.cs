...
/// </summary>
public static class ScryvDrawingViewService
{
	/// <summary>
	...
	/// <param name="token"><see cref="CancellationToken"/></param>
	/// <returns>Image stream</returns>
	public static ValueTask<Stream> GetImageStream(IList<ScryvInkPoint> points, Size imageSize, float lineWidth, Color strokeColor, Paint? background, CancellationToken token = default)
	{
		token.ThrowIfCancellationRequested();
	...
	static UIImage? GetUIImageForPoints(ICollection<ScryvInkPoint> points,
		NFloat lineWidth,
		Color strokeColor,
		Paint? background)
	{
	...
	static UIImage? GetUIImage(ICollection<ScryvInkPoint> points, Action<CGContext, Size> drawStrokes, Paint? background, NFloat maxLineWidth)
	{
		const int minSize = 1;
	...
	static void DrawStrokes(CGContext context, IList<ScryvInkPoint> points, NFloat lineWidth, Color strokeColor, Size offset)
	{
