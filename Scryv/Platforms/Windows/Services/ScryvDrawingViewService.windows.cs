...
public static class ScryvDrawingViewService
{
	...
	static CanvasRenderTarget? GetImageInternal(ICollection<ScryvInkPoint> points,
		Size size,
		float lineWidth,
		Color lineColor,
		Paint? background,
	...
	static void DrawStrokes(CanvasDrawingSession session,
		IEnumerable<ScryvInkPoint> points,
		Color lineColor,
		float lineWidth,
		Size offset)
	{
	...
	static (CanvasRenderTarget? offscreen, Size offset) GetCanvasRenderTarget(ICollection<ScryvInkPoint> points, SizeF size, bool scale, float maxLineWidth)
	{
		const int minSize = 1;
