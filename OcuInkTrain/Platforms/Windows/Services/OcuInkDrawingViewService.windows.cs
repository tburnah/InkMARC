...
public static class OcuInkDrawingViewService
{
	...
	static CanvasRenderTarget? GetImageInternal(ICollection<OcuInkPoint> points,
		Size size,
		float lineWidth,
		Color lineColor,
		Paint? background,
	...
	static void DrawStrokes(CanvasDrawingSession session,
		IEnumerable<OcuInkPoint> points,
		Color lineColor,
		float lineWidth,
		Size offset)
	{
	...
	static (CanvasRenderTarget? offscreen, Size offset) GetCanvasRenderTarget(ICollection<OcuInkPoint> points, SizeF size, bool scale, float maxLineWidth)
	{
		const int minSize = 1;
